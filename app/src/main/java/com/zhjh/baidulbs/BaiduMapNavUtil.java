...
public class BaiduMapNavUtil {  
    ...
    public static BaiduAMapNavUtil getInstance(Context con){  
        if(instance == null){  
            context = con;  
            instance = new BaiduAMapNavUtil();  
        }  
        return instance;  
    }  
      
    //发起百度驾车导航  
    public void goToBaiduNaviActivity(Context context,double lat,double lng){  
    ...
    private boolean isAvilibleMap(int flag){     
        ...
        if(flag == 1){  
            ...
            st = packageNames.contains(AMapPackage);    
        }  
        return st;  
   }   
     ...
     * 获取可用的导航列表 
     */  
    public List<NavModel> getAvailableNavi(){  
        List<NavModel> list = new ArrayList<NavModel>();  
        int flag = 1;  
        NavModel n = null;  
        if(isAvilibleMap(flag)){  
            n = new NavModel();  
            n.setFlag(flag);  
            n.setName("百度驾车导航");  
            list.add(n);  
